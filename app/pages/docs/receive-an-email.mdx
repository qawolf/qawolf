...
In this tutorial we will receive an email in a test. This is helpful for scenarios like:

- Sign in with a login code
- Password reset
- Email notifications after an event happens

<SubHeader>Create Email Address</SubHeader>
...
You can optionally pass `{ new: true }` to `getInbox` to get a unique email address.

```js
const { email, waitForMessage } = getInbox(); // default inbox for your team
const { email, waitForMessage } = getInbox({ new: true }); // unique inbox
```

<SubHeader>Use Email Address</SubHeader>

You can now use the email address in your test. For example, at QA Wolf we use `getInbox` in our sign in test, since our sign in requires a login code.

In the example below, we 1) create a email address, 2) fill out the sign in form, and 3) get the login code from the email subject line.
