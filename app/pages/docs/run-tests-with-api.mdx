...
- The <ExternalLink href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Authorization">Authorization header</ExternalLink> provides your team's API key: `-H "Authorization: API_KEY_HERE"`.
- The optional `env` object in the request body passes additional environment variables to your tests: `"env": { "MY_VARIABLE": "secret" }`.
- The optional `env_name` string in the request body specifies which <Link href="use-environment-variables#create-an-environment">environment</Link> to use when running your tests: `"env_name": "Staging"`. If you have multiple environments and do not provide an `env_name`, QA Wolf will error and ask you to specify an environment.
