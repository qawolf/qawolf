...
QA Wolf maintains a <ExternalLink href="https://github.com/qawolf/netlify-plugin-qawolf">Netlify Build Plugin</ExternalLink> that runs your tests on <ExternalLink href="https://www.netlify.com/blog/2016/07/20/introducing-deploy-previews-in-netlify">deploy previews</ExternalLink> or production deployments. If you have a different workflow, please <ExternalLink href="https://slack.qawolf.com">reach out for help</ExternalLink>.
...
First we need to install the QA Wolf Build Plugin. There are two ways to do this: from the Netlify UI (recommended) or with <ExternalLink href="https://www.npmjs.com/">npm</ExternalLink>.

### A. Install from the Netlify UI (Recommended)

Click <ExternalLink href="https://app.netlify.com/plugins">
this link to install the QA Wolf Build Plugin</ExternalLink>.
...
<SubHeader>Choose Whether to Block Deployments</SubHeader>

You can also optionally choose to block deployments if your tests fail. This option is presented under the label **Block deployment if tests fail?**

If you choose **No (run tests onSuccess)**, your tests will run after your deployment succeeds. This means that even if your tests fail, your deployment will still be live.

If you choose **Yes (run tests onPostBuild)**, your tests will run after your site builds but before it is deployed. This means that if your tests fail, your build will not be deployed. Only if your tests pass will your deployment be live.

Learn more about Netlify events <ExternalLink href="https://docs.netlify.com/configure-builds/build-plugins/create-plugins/#plug-into-events">here</ExternalLink>.

<Image
  alt="Block Netlify deployment"
  height={65.5}
  src={`/docs/run-tests-on-netlify-deployment/block-deployment.png`}
  width={488}
/>
