...
import { updateUserLastSeenAt } from "../../../server/models/user";
...
describe("updateUserLastSeenAt", () => {
  it("updates last_seen_at on user", async () => {
    const last_seen_at = await updateUserLastSeenAt("spirit", { db, logger });

    const user = await findUser({ id: "spirit" }, { db, logger });

    expect(user).toMatchObject({ last_seen_at: new Date(last_seen_at) });
  });
});
