...
By default, your tests will whenever someone pushes a commit to any branch.

The rest of this documentation covers common use cases of running your tests in CI:
...
Your debugging artifacts are uploaded in a folder called `artifacts`, which has the following structure. Each test has its own folder for its artifacts, with the folder named after the test like `myTestName.test.js`.
...
The `qawolf_{timestamp}.log` file contains detailed logs from the QA Wolf Node.js server. You probably won't need to look at it (unless you are submitting it to us for help deubgging an issue 😊).
