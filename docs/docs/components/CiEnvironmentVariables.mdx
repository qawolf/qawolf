...
If your environment variable is not sensitive, you can set its value directly. To do this, just add another line under the `env` key. In the example below, we set [`QAW_SLEEP_MS`](TODOFIXLINK) to `0` so that our tests run as fast as possible:
...
In our test file, we use `process.env.LOGIN_PASSWORD` rather than hard coding the password value. See [our guide on changing input values](TODOFIXLINK) to learn more about removing sensitive input values from tests.
