...
The element will be cleared before typing if value does not start with `Enter` or `Tab` keys and `options.skipClear` is `false`.

### Arguments

<ArgumentSelector />
<ArgumentTypeValue />
<ArgumentFindElementOptions delayMs skipClear />
...
Type `"cat"` in the element with the CSS class `"best-pet"`. Don't clear the element before typing and time out if the element is not found within 5 seconds:
