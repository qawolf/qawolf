...
- You can disable watch mode in [qawolf.config.js](config#watch) and the test will not re-run on save
- You can comment in/out `await qawolf.create();` to turn code creation on/off
