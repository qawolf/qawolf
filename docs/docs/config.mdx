...
If it finds one, it will install these additional dependencies: `npm i -D @types/debug @types/jest @types/node ts-jest` and set these properties in `qawolf.config.js`:
