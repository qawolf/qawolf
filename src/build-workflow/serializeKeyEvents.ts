...
export const isKeyHeld = (events: KeyEvent[], thresholdMs = 500): boolean => {
  /**
  ...
  for (let i = 0; i < events.length - 1; i++) {
    ...
    if (heldTime > thresholdMs) {
      debug(`${keydownEvent.value} was held for ${heldTime}`);
      return true;
    }
  }
...
export const keyToCode = (key: string): string => {
  ...
  if (!code) throw new Error(`No code found for ${key}`);
...
export const serializeCharacterStrokes = (events: KeyEvent[]): string => {
   ...
   */
  debug('serializeCharacterStrokes');
...
export const serializeKeyEvents = (events: KeyEvent[]): string => {
  ...
  if (hasNotUSKey) {
    debug('serializeKeyEvents: not US key found');
